...
urlpatterns = [
    ...
    path('<int:forum_id>/threads/', ThreadViewSet.as_view({'get': 'list'}), name='thread-list'),
    path('<int:forum_id>/threads/<int:pk>/', ThreadViewSet.as_view({'get': 'retrieve'}), name='thread-detail'),
    path('<int:forum_id>/threads/<int:thread_id>/posts/', PostViewSet.as_view({'get': 'list'}), name='forum-post-list'),
